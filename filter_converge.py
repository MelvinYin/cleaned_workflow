...
class Filter:
    ...
    def run(self):
        to_run = (self.screen_correlated, self.screen_non_combi)
        for func in to_run:
            try:
    ...
    def delete_intermediate(self):
        for file in self._dir:
            self.to_trash(file)
        if self.cluster:
            self.cluster.delete_intermediate()
        return
    ...
    def screen_correlated(self):
        ...
        command = f"{self.dir.meme_dir}/mast -remcorr " \
                  f"{self.dir.orig} {self.dir.single_seq} -o " \
    ...
    def screen_non_combi(self):
        ...
        self.cluster = Cluster(cluster_dir)
        self.cluster.run()
