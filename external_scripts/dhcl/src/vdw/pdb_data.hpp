...
inline AtomVector parseAtomLines(FILE* fh)
{
  ...
  while ( fgets (line, 200, fh ) != NULL)
	...
	continue;	
      long atom_number = atol( sline.substr(6,5).c_str() );
      string atom_name = removeSpaces(sline.substr(12,4));
      char alt_loc = sline[16];
      string res_name = removeSpaces(sline.substr(17,3));
      char chain = sline[21];
      long res_number = atol( sline.substr(22,4).c_str() );
      char icode = sline[26];
      double x = atof( sline.substr(30, 8).c_str() );
      double y = atof( sline.substr(38, 8).c_str() );
      double z = atof( sline.substr(46, 8).c_str() );
